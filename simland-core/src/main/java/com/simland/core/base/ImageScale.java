...
public class ImageScale {
	...
	private static final Log logger = LogFactory.getLog(ImageScale.class);
	
	public static final String[] imgSize = new String[] { "365,472", "730,944", "85,110", "170,220", "56,73", "33,44" };
	...
	public static void main(String[] args) {
		String[] urls = new String[imgSize.length];
		BufferedImage image = null;
		try {
			image = ImageIO.read(new File("E://show//f.jpg"));
		} catch (IOException e) {
		...
		try {
			for (int i = 0; i < imgSize.length; i++) {
				outImpage = is.imageZoomOut(image, Utils.parseInt(imgSize[i].split(",")[0], 100), Utils.parseInt(imgSize[i].split(",")[1], 100));
				urls[i] = "D://b//e_" + i + ".jpg";
				File f = new File(urls[i]);
				ImageIO.write(outImpage, "jpg", f);
			}
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
